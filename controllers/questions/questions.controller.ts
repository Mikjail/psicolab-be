...
export class QuestionsController {
  ...
  @UseHook(MyHook, 'payload')
    ...
    async getQuestions() {
      try {
        const text = await Answer.select('question_id', 'text', 'answers.id').join(Question, Question.field('id'), Answer.field('question_id')).all();
        ...
        const questionWithAnswer = questions.map( (question:any) => ({
          ...
          .map( (answer:any) => ({ answer: answer.text, id: answer.id}))
        }))
